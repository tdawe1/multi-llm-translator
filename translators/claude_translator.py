...
def translate_with_claude(text: str, target_language: str, source_language: str = "English") -> str:
    ...
    prompt_placeholders = {'{source_language}': source_language, '{target_language}': target_language, '{text}': text}
    ...
    try:
        message = client.messages.create(model="claude-3-5-sonnet-20240620", max_tokens=4096, system=system_prompt, messages=[{"role": "user", "content": text}])
...
def critique_with_claude(source_text: str, primary_translation: str, target_language: str, source_language: str = "English") -> str:
    ...
    prompt_placeholders = {'{source_language}': source_language, '{target_language}': target_language, '{source_text}': source_text, '{primary_translation}': primary_translation}
    ...
    try:
        message = client.messages.create(model="claude-3-5-sonnet-20240620", max_tokens=4096, messages=[{"role": "user", "content": critique_prompt}], temperature=0.4)
        return message.content[0].text
    except Exception as e:
