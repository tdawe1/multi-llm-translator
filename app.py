...
if DUMMY_MODE:
    ...
    critique_with_gpt = dummy_critique_with_gpt
    translate_with_claude = dummy_translate_with_claude
    critique_with_claude = dummy_critique_with_claude
    translate_with_gemini = dummy_translate_with_gemini
    critique_with_gemini = dummy_critique_with_gemini
...
def save_translation_output(source_filepath, service, result, job_id=None):
...
def csv_handshake_worker():
    ...
    while True:
        ...
        for _, job in new_jobs.iterrows():
            job_link = job['link']; job_id = get_job_id_from_link(job_link)
            ...
            if source_filepath:
                ...
                if OPERATION_MODE == 'SIMPLE':
                    ...
                    for service, result in translations.items():
                        if not result.startswith("Error:"):
                            save_translation_output(source_filepath, service, result, job_id); success_count += 1
                    total_tasks = len(models)
                    if success_count == total_tasks: logger.info(f"[Job {job_id}] Processing complete. All {total_tasks} translations succeeded.")
                    elif success_count > 0: logger.warning(f"[Job {job_id}] Processing complete with partial success. Succeeded: {success_count}/{total_tasks}.")
                    else: logger.error(f"[Job {job_id}] Processing failed. All {total_tasks} translations failed.")
                ...
                log_processed_job(job_link); logger.info(f"[Job {job_id}] Finished & Logged.")
            else:
                logger.warning(f"[Job {job_id}] Timed out. File not found. Assuming job was rejected."); log_processed_job(job_link)
...
class HotFolderHandler(FileSystemEventHandler):
    def on_created(self, event):
        ...
        source_filepath = event.src_path; filename = os.path.basename(source_filepath)
        job_id_match = re.match(r'^(\d+)', filename)
        if job_id_match and job_id_match.group(1) in active_handshake_jobs:
            logger.info(f"Hot Folder: Ignoring '{filename}' (handled by CSV worker)."); return
        ...
        for service, result in translations.items():
            if not result.startswith("Error:"):
                save_translation_output(source_filepath, service, result); success_count += 1
        
        total_tasks = len(models)
        if success_count == total_tasks: logger.info(f"Hot Folder Job '{filename}': Processing complete. All {total_tasks} translations succeeded.")
        elif success_count > 0: logger.warning(f"Hot Folder Job '{filename}': Processing complete with partial success. Succeeded: {success_count}/{total_tasks}.")
        else: logger.error(f"Hot Folder Job '{filename}': Processing failed. All {total_tasks} translations failed.")
